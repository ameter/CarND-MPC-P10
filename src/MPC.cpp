 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i = 0; i < N - 1; i++) {
      ...
      // Calculate polynomial value with coefficients that were passed in.
      AD<double> f0 = coeffs[0] + coeffs[1] * x0 + coeffs[2] * x0 * x0 + coeffs[3] * x0 * x0 * x0 ;
      // Calculate desired psi
      AD<double> desiredPsi0 = CppAD::atan(3*coeffs[3]* x0 * x0 + 2*coeffs[2]* x0 + coeffs[1]);
      ...
      fg[2 + epsi_start + i] = epsi1 - ((psi0 - desiredPsi0) - v0 * delta0 / Lf * dt);
    }
  }
};
